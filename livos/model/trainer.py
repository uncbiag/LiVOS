...
from livos.model.utils.parameter_groups import get_parameter_groups
from livos.model.losses import LossComputer
from livos.utils.log_integrator import Integrator
from livos.utils.image_saver import vis
from livos.utils.logger import TensorboardLogger
from livos.utils.time_estimator import TimeEstimator
...
class Trainer:
    def __init__(
        ...
        self.use_amp = stage_cfg.amp and False

        local_rank = torch.distributed.get_rank()
        self.local_rank = local_rank

        # setting up the model
        self.livos = nn.parallel.DistributedDataParallel(
            LIVOS().cuda(),
        ...
        self.log.log_string(
            'model_size', 
            str(sum([param.nelement() for param in self.livos.parameters()])))

        self.log.log_string(
            'number_of_parameters_that_require_gradient',
            str(sum([param.nelement() for param in filter(lambda p: p.requires_grad, self.livos.parameters())])))
        ...
        parameter_groups = get_parameter_groups(
            self.livos, stage_cfg, print_log=(local_rank == 0))
    ...
    def do_pass(self, data, it=0):
        ...
        if self._is_train:
            losses = self.loss_computer.compute({**data, **out}, num_objects)
            ...
            if self._is_train:
                if self.local_rank == 0 and it % self.log_image_interval == 0 and it != 0:
                    images = {**data, **out}
                    self.log.log_image(
                        self.stage, 'vis', vis(images, self.size, num_objects), it)
        ...
        if self.use_amp:
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(self.livos.parameters(), self.clip_grad_norm)
    ...
    def save_weights(self, it):
        ...
        torch.save(self.livos.module.state_dict(), model_path)
    ...
    def save_checkpoint(self, it):
        ...
        checkpoint = {
            'it': it,
            'weights': self.livos.module.state_dict(),
    ...
    def load_checkpoint(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        self.livos.module.load_state_dict(weights)
    ...
    def load_weights_in_memory(self, src_dict):
        self.livos.module.load_weights(src_dict)
    ...
    def weights(self):
        return self.livos.module.state_dict()
